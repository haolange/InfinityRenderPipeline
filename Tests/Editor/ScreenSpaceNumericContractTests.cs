...
namespace InfinityTech.Rendering.Pipeline.Tests
{
    public partial class GBufferContractTests
    {
        ...
        public void ScreenSpaceNumericContract_ProductionKernels(int width, int height)
        {
            ...
            File.WriteAllText(Path.Combine(output, "PASS.txt"), $"13 production kernels; full={width}x{height}; half={Mathf.Max(1, width >> 1)}x{Mathf.Max(1, height >> 1)}; raw finite, active writes, padded dispatch guards, nonzero trace and chain.\n");
        ...
        sealed class ScreenSpaceNumericGpuRun : IDisposable
        {
            ...
            public ScreenSpaceNumericGpuRun(int width, int height, string output)
            {
                ...
                // Synthetic clip-space plane, with consistent identity projective matrices.
                // Tilted normals avoid a deliberately singular screen-parallel ray direction.
                Vector3 normal = new Vector3(0.25f, 0.35f, -0.90f).normalized;
                m_Depth = Solid(width, height, new Color(0.5f, 0, 0, 0));
            ...
            public void RunReflection()
            {
                ...
                RequireSignal(ray[1], "SSR actual ray radiance", 0.01f);
                AssertKnownReflectionColor(ray[1]);
                ...
                Matrix4x4 jittered = Matrix4x4.Translate(new Vector3(0.03125f, -0.046875f, 0));
                ...
                RequireSignal(bilateral, "SSR final radiance", 0.01f);
            ...
            public void RunIndirectDiffuse()
            {
                ...
                RequireSignal(ray, "SSGI actual trace irradiance", 0.01f);
                Texture2D spatial = Dispatch(shader, "SpatialFilter", m_Width, m_Height, false,
                ...
                RequireSignal(result, "SSGI final irradiance", 0.01f);
            }
            ...
            public void RunOcclusion()
            {
                ...
                Texture2D depth = Solid(width, height, new Color(0.5f, 0, 0, 0));
                Texture2D normal = Solid(width, height, new Color(0.5f, 0.5f, 0, 1));
                ...
                Texture2D temporal = Dispatch(shader, "OcclusionTemporal", width, height, true,
                    new[] { "UAV_AccmulateTexture" }, ("SRV_DepthTexture", depth), ("SRV_OcclusionTexture", spatialY),
                ...
                Texture2D result = Dispatch(shader, "OcclusionUpsample", m_Width, m_Height, true,
                    new[] { "UAV_UpsampleTexture" }, ("SRV_DepthTexture", m_Depth),
                    ("SRV_NormalTexture", Solid(m_Width, m_Height, new Color(0.5f, 0.5f, 0, 1))), ("SRV_OcclusionTexture", temporal))[0];
            ...
            static void SetMatrices(ComputeShader shader)
            {
                foreach (string name in new[] { "Matrix_Proj", "Matrix_InvProj", "Matrix_InvViewProj", "Matrix_LastViewProj", "Matrix_WorldToView", "Matrix_ViewToWorld", "Matrix_HitMotionViewProj", "Matrix_HitMotionLastViewProj" })
                ...
                string[] outputs, params (string name, Texture texture)[] inputs)
            {
                ...
                {
                    cmd.SetGlobalVector("_ZBufferParams", new Vector4(0, 1, 1, 1));
            ...
            void AssertKnownReflectionColor(Texture2D texture)
            {
                int hits = 0;
                foreach (Color color in texture.GetPixels())
                {
                    if (color.r <= 0.01f) continue;
                    ++hits;
                    Assert.That(color.r, Is.EqualTo(0.8f).Within(0.002f), "SSR known radiance R");
                    Assert.That(color.g, Is.EqualTo(0.4f).Within(0.002f), "SSR known radiance G");
                    Assert.That(color.b, Is.EqualTo(0.2f).Within(0.002f), "SSR known radiance B");
                }
                Assert.Greater(hits, 0, "Known reflection fixture must hit; all-zero output cannot pass");
                m_Report.AppendLine("PASS known reflection radiance; hits=" + hits);
            }
