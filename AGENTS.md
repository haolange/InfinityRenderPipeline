...
6. **SceneView temporal gating.** A new `CameraFrameState` or `Time.frameCount - lastSeenFrame > 1` forces `historyReset` and that frame has no jitter (`Matrix_ViewJitterProj == Matrix_ViewProj`). SceneView states linger 120 unseen frames; Game stays 8. Preview cameras keep the pre-T5 path (always jitter, 8-frame recycle).
