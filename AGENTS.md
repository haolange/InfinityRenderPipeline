...
## Active Editor convergence override (user approved)

The current C0-C7 plan in PLAN.md supersedes prior normalization roles and incompatible target contracts below. Main agent implements and visually inspects; independent GPT-6 Astra verifies. Luna is search-only. Do not create branches, commit, push, build or package Players. All Editor quality gaps are included.

Targets under implementation: native Renderers use Unity previous motion data without NativeViewMotionHistory/MPB pose ownership; custom MeshDrawing retains necessary history. TAA and output sharpening fuse into one dispatch while history remains unsharpened. DisplayColorBuffer is display-linear, with encoding in final Present. Previously described separate sharpen/output-transform and native pose snapshot paths are current legacy implementation facts to retire after verified replacement, not authority to retain them. Only confirmed view-history mismatches reset the affected view.

Every C stage requires complete source/asset identity, self-tests, applicable actual-frame evidence, independent Astra PASS and owned intermediate cleanup. Historical N/U PASS receipts do not close these new gates. Do not reduce the approved gate to candidate-local booleans.

Verified SceneView correction (2026-09-13): the display UI overlay list is recorded only for `CameraType.Game && camera.targetTexture == null`. Never restore SceneView to that consumer gate. Explicit Unity-owned camera targets are imported by identifier with their descriptor; graph disposal releases only the wrapper. Dimensions freeze after BeginCameraRendering callbacks. The independent OutputTransform compute pass/kernel/marker/resource field have been retired; encoding is in final Present. See `Docs/Editor-Convergence-SceneView-Size.md` for source/asset identity, actual-frame evidence and open gates. Native AssetDatabase/Editor recovery failures do not invalidate recorded history or authorize a completion claim.
...
- RTAO records only when UnifiedRayTracing `RayTracingContext` + `IRayTracingAccelStruct` + `VisibilityRTAO.urtshader` are ready and the accel has instances. Dispatch goes through `IRaytracingCommands` (`BuildUnifiedAccel` / `DispatchUnified`). HiZ screen-space `Compute_RayTracedAmbientOcclusion` and the old `RayTracing*Generator` owners are deleted. If URT cannot initialize, RTAO does not record and AO falls back to GTAO; Debugger shows `RTAO Record Reason`. Player shader load stays `TODO(UNVERIFIED)` (Editor `LoadRayTracingShader` only). D3D12 hardware is `UNVERIFIED (external)`.
...
1. **`EnableNativeRenderPass(false)` is allowed only where a native-RP attachment is impossible.** Today that is Gizmo / WireOverlay (Unity forbids drawing gizmos inside `BeginRenderPass`), Present (the backbuffer has no owning `RenderTexture`, so `AttachmentDescriptor.graphicsFormat` cannot be resolved), and UI Overlay (`CreateUIOverlayRendererList` / `DrawRendererList` is uGUI's overlay list; Unity does not expose that list as a native-RP attachment payload). A new use needs the same kind of hard API reason, not convenience. Overlay is not `DrawUIOverlay`.
...
1. **Resources live only in GlobalSettings `IRenderPipelineResources` containers** with `[ResourcePath]`. The RP Asset carries no shader, blit-material, or LUT-texture references. Missing required resources fail at record/setup. `ResolveDefaultVolumeProfile` throws when the GlobalSettings slot is empty; there is no `Resources.Load` fallback. Player shader stripping lives in `Editor/ShaderStripper/` (`IPreprocessBuildWithReport` + Infinity `IPreprocessShaders` / `IPreprocessComputeShaders`).
...
4. **uGUI is a first-class consumer.** `SupportedRenderingFeatures.active.rendersUIOverlay = true`. Screen Space Overlay uses `CreateUIOverlayRendererList` + `DrawRendererList` after OutputTransform (`UIOverlayPass`). Screen Space Camera / World Space canvases use the translucent RendererList. Missing overlay support is a contract defect, not an optional skip.
