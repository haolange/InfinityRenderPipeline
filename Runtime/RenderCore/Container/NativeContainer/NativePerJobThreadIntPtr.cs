...
{
	/// <summary>
	/// A pointer to an int stored in native (i.e. unmanaged) memory. One
	/// integer is stored for each of the maximum number of job threads. As of
	/// Unity 2018.2, this results in 8 KB of memory usage. The advantage over
	/// <see cref="NativeIntPtr"/> is that all operations on
	...
	[DebuggerTypeProxy(typeof(NativePerJobThreadIntPtrDebugView))]
	...
	public unsafe struct NativePerJobThreadIntPtr : IDisposable
	{
		...
		/// </param>
		public NativePerJobThreadIntPtr(Allocator allocator, int initialValue = 0)
		{
		...
		public int Value
		{
			get
			{
				...
				{
					value += m_Buffer[IntsPerCacheLine * i];
				}
				return value;
			}
